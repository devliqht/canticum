      ...
      if (accessToken) {
        ...
        return (
            ...
            <div className="max-w-screen-lg w-full rounded-lg p-4" style={{ backgroundImage: backgroundGradient }} ref={containerRef}>
              ...
              <h1 className="text-3xl font-bold">{timeRange === 'short_term' && 'LAST MONTH'} {timeRange === 'medium_term' && 'LAST SIX MONTHS'} {timeRange === 'long_term' && 'ALL TIME'}</h1>
              <h1 className="text-2xl"><span className="text-secondary">{userName}'s</span> Top {selectedOption === 'tracks' && 'songs'}{selectedOption === 'artists' && 'artists'}{selectedOption === 'genres' && 'genres'} </h1>  
              </div>
                    ...
                    <div key={track.id} className="relative lg:h-full shadow-md lg:rounded-lg overflow-hidden transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                      ...
                      <div className=" text-white text-sm px-4 py-2 absolute bottom-0 w-full flex flex-row gap-4">
                          ...
                          <div className="flex flex-col">
                              <span className="text-2xl">{track.name}</span> <span className="text-neutral-400">
              ...
              { simpleModeEnabled &&
                  ...
                  <tbody>
                    {selectedOption === 'tracks' && topTracks.map((track, index) => (
                      <tr key={track.id}>
                        <td className="py-2 pr-4 text-center font-bold">{index + 1}</td>
                        <td className="py-2"><span className="text-xl"><strong>{track.name}</strong></span><br></br>
                    ...
                    {selectedOption === 'artists' && topArtists.map((artist, index) => (
                      <tr key={artist.id}>
                        <td className="py-2 pr-4 text-center font-bold">{index + 1}</td>
                        <td className="py-2"><span className="text-xl"><strong>{artist.name}</strong></span><br></br>
